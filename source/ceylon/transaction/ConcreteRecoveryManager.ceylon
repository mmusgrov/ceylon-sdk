...
class ConcreteRecoveryManager() satisfies TransactionRecoveryManager {
    shared actual void start(String? objectStoreDir) {
        if (exists objectStoreDir) {
            ...
            print("Will recover transaction logs located under ``getProperty("com.arjuna.ats.arjuna.common.ObjectStoreEnvironmentBean.objectStoreDir", ".")``");
        }

        if (!rm exists) {
            xARecoveryModule = XARecoveryModule();
            recoveryEnvironmentBean.recoveryModules
                    = Arrays.asList<RecoveryModule>(AtomicActionRecoveryModule(), xARecoveryModule);
    ...
    shared actual void recoveryScan() {
        rm?.scan();
    }
